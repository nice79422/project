...
<title>수주 관리</title>
  <%-- <jsp:include page="/decorator/head.jsp"/> --%>
	...
	$(document).ready(function(){
		/* getContractList(); */
		$("#addContractB, #addOrderB, #batchProcessB, #saveOrderB,#addAutoContractB,#addEstimateOrderB").button().css("font-size","15px");
	...
	function contractReviewListGrid(contractItemList){	//수주리스트 가져오기
	...
	function getOrderList(contractCode){
		...
		$('#orderListTable').jqGrid({
			...
			colModel:[
				...
				{name:'itemCode',width:130,align:"center",editable:false},
	...
	function codeInfo(baseCode){
		...
		$("#showCode").jqGrid({
			...
			onSelectRow : function(id){
				if(baseCode=='LO-08'){
				$("#orderListTable").setCell(row, "itemCode", $("#showCode").getCell(id, 'detailCode'));
	...
	function saveOrderList(){//수주리스트와 주문리스트를 CTO 형식으로 담음
		...
		for(var index in orderListIds){
			...
			contractDetailBean.itemCode=orderData.itemCode;
	...
	function standByContract(newEstimate){
		$.ajax({
		    url : "${pageContext.request.contextPath}/logistics/business/contract.do",
		    contentType : "application/x-www-form-urlencoded; charset=UTF-8",
		    type : 'post',
...
/* 		    	alert("견적의 처리여부가  Y로 변경되었습니다."); */
	...
	function estimateListGrid(){
		...
		$("#showEstimate").jqGrid({
							...
							ondblClickRow : function(rowid) {
								...
								//수주에견적적용
								var conData=$("#showEstimate").getRowData(rowid);
								var newRowNum = $('#contractListTable').jqGrid('getDataIDs').length+1;
								$('#contractListTable').addRowData(newRowNum,{"produceStatus":"N","estimateCode":conData.estimateCode,"standByStatus":"N"});
	...
	function applyEstimate(){
		...
		var newRowNum = $('#orderListTable').jqGrid('getDataIDs').length+1;
		$('#orderListTable').addRowData(newRowNum,{"mpsStatus":"N","clientCode":estimateDetailList[i].clientCode,"contractQuantity":estimateDetailList[i].estimateAmount,"itemCode":estimateDetailList[i].itemCode});
		}
	}
