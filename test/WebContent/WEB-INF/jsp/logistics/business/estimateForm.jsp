...
<title>견적 관리</title>
  ...
  $(document).ready(function(){
	/* getEstimateList(); */
    /* getEstimateDateList(); */
    $("#addEstimateB, #addEstimateDetailB, #batchProcessB, #saveEstimateDetailB, #pdfB ").button().css("font-size","10px");
  ...
  function codeInfo(baseCode,baseName){
    ...
    $("#showCode").jqGrid({
      ...
      onSelectRow : function(id){
        ...
        $("#estimateDetailListTable").setCell(row, "itemBean.itemCode", $("#showCode").getCell(id, 'detailCode'));
      }
	  ...
	  var selectedRowId =  $("#estimateListTable").getGridParam('selrow');
		...
		for(var index in estimateDetailListIds){
	     var estimateDetailData=$("#estimateDetailListTable").getRowData(estimateDetailListIds[index]);
	      ...
	      estimateDetailBean.itemCode=estimateDetailData['itemBean.itemCode'];
...
/* 	    	alert("견적의 처리여부가  Y로 변경되었습니다."); */
	...
	function getPdfForm(){//pdf 출력하기
		...
		var estimateCode=estimateData.estimateCode;
		alert(estimateCode);
	    window.open("${pageContext.request.contextPath}/report.do?method=estimateReport&orderDraftNo="+estimateCode, "window", "width=1600, height=1000");
	 }
  </script>
</head>
<body>
