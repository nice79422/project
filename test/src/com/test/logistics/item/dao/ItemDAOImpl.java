...
public class ItemDAOImpl implements ItemDAO {
	 private static ItemDAOImpl instance;
	    ...
	    public List<ItemBean> selectItemList(String code) throws DataAccessException {
	    	if(logger.isDebugEnabled()){logger.debug("selectItemList() - 시작");}
	    	...
	    	try{
	            ...
	            if(logger.isDebugEnabled()){logger.debug("selectItemList() - 끝");}
	            return itembean;
	    	}catch(Exception e){
				if (logger.isFatalEnabled()) { logger.fatal("selectItemList(...)-에러");}
	    ...
	    public List<ItemBean> selectItemList(int sr, int er, String searchWord)
	    	...
	    	try {
	    		...
	    		if (logger.isDebugEnabled()) {
	    			logger.debug("selectEmpList() - 끝");
	    		}
	    	
	    		return itemBean;
	    	} catch (Exception e) {
	    		logger.fatal("selectEmpList()  - 에러");
	    		throw new DataAccessException(e.getMessage());
	    	} finally {
	    		dataSourceTransactionManager.close(pstmt);
	    	}
	    ...
	    public void insertItem(ItemBean item) throws DataAccessException {
	    	String[] data = null;
	    	String code = null;
	    	if(logger.isDebugEnabled()){logger.debug(" insertItem() - 시작");}
	    	Connection con=null;
	    	PreparedStatement pstmt = null;
	    	ResultSet rs = null;
	    	try{
	    		...
	    		if(logger.isDebugEnabled()){logger.debug("insertItem() - 끝");}
	    	}catch(Exception error){
				if (logger.isFatalEnabled()) { logger.fatal("insertItem(...)-에러");}
	    ...
	    public void updateItem(ItemBean item){
	    	if(logger.isDebugEnabled()){logger.debug("updateItem() - 시작");}
	    	Connection con = null;
	    	PreparedStatement pstmt = null;
	    	try{
	    		StringBuffer query=new StringBuffer();
	    		...
	    		/* pstmt.setString(1, item.getItemName());
	    		 ...
	    		 pstmt.executeUpdate();
	    		 if(logger.isDebugEnabled()){logger.debug("updateItem() - 끝");}
	    	}catch(Exception sqle){
				if (logger.isFatalEnabled()) { logger.fatal("updateItem(...)-에러");}
	    		throw new DataAccessException(sqle.getMessage());
	    	}finally{
	    		dataSourceTransactionManager.close(pstmt);
	    	}
	    ...
	    public void deleteItem(ItemBean item){
	    	if(logger.isDebugEnabled()){logger.debug("deleteItem() - 시작");}
	    	Connection con=null;
	    	PreparedStatement pstmt = null;
	    	try{
	    		StringBuffer query=new StringBuffer();
	    		query.append("DELETE ITEM WHERE ITEM_NO=?");
	    		 ...
	    		 pstmt.setString(1, item.getItemCode());
	    		 pstmt.executeUpdate();
	    		 if(logger.isDebugEnabled()){logger.debug("deleteItem() - 끝");}
	    	}catch(Exception e){
				if (logger.isFatalEnabled()) { logger.fatal("deleteItem(...)-에러");}
	    		throw new DataAccessException(e.getMessage());
	    	}finally{
	    		dataSourceTransactionManager.close(pstmt);
	    	}
