...
public class EmpDAOImpl implements EmpDAO {
    ...
    public void insertEmp(EmpBean emp) throws DataAccessException {
 	 if(logger.isDebugEnabled()){logger.debug("insertEmp() - 시작");}
      Connection con = null;
      PreparedStatement pstmt = null;
      ResultSet rs=null;
      try{
          ...
          if(logger.isDebugEnabled()){logger.debug("insertEmp() - 끝");}
      }catch(Exception error){
  		if(logger.isFatalEnabled()){ logger.fatal("insertEmp()  - 에러");}
	...
	public EmpBean selectEmp(String empCode) throws DataAccessException {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("selectMember() - 끝");
			}
			return emp;
		} catch (Exception sqle) {
			if (logger.isFatalEnabled()) {
				logger.fatal("selectMember()  - 에러");
			}
			throw new DataAccessException(sqle.getMessage());
		} finally {
			dataSourceTransactionManager.close(pstmt);
	   ...
	   @Override
	    public void updateEmp(EmpBean emp){
	    	if(logger.isDebugEnabled()){logger.debug("updateEmp() - 시작");}
	        Connection con = null;
	        PreparedStatement pstmt = null;
	        try{
	            StringBuffer query=new StringBuffer();
	            ...
	            if(logger.isDebugEnabled()){logger.debug("updateEmp() - 끝");}
	        }catch(Exception sqle){
	     		if(logger.isFatalEnabled()){ logger.fatal("updateEmp()  - 에러");}
	            throw new DataAccessException(sqle.getMessage());
	        }finally{
	        	dataSourceTransactionManager.close(pstmt);
	        }
    ...
    public void deleteEmpList(String empCode) throws DataAccessException {
   	   if(logger.isDebugEnabled()){logger.debug("deleteEmpList() - 시작");}
   	   Connection con = null;
  		   PreparedStatement pstmt = null;
  		   try{
  			StringBuffer query = new StringBuffer();
			...
			pstmt = con.prepareStatement(query.toString());
				pstmt.setString(1, empCode);
				pstmt.executeUpdate();
				if(logger.isDebugEnabled()){logger.debug("deleteEmpList() - 끝");}
  		   }catch(Exception sqle){
        		if(logger.isFatalEnabled()){ logger.fatal("deleteEmpList()  - 에러");}
  			   throw new DataAccessException(sqle.getMessage());
  		   }finally{
  			dataSourceTransactionManager.close(pstmt);
  		   }
